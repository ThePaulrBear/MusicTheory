...
public class Progression {
	...
	private void calculateKey(List<AbsoluteChord> chordsInProgression){
		...
		for(Key key : Key.getAllKeys()){
			...
			RomanNumeralChord rn = key.convertAbsoluteChordToRomanNumeral(chordsInProgression.get(i));
		...
		while(i > 0){
			...
			for(PossibleKey key : possibleKeys){
				RomanNumeralChord rn = key.getKey().convertAbsoluteChordToRomanNumeral(chordsInProgression.get(i));
				if(rn == null) {
					continue;
				}
	...
	private static AbsoluteRomanNumeralChord getAbsoluteRomanNumeral(
		...
		RomanNumeralChord rn = key.convertAbsoluteChordToRomanNumeral(chord);
